...
func (m *BlobParameterValue_InlinedBytes) CloneVT() isBlobParameterValue_BlobOneof {
	if m == nil {
		return (*BlobParameterValue_InlinedBytes)(nil)
	}
	r := new(BlobParameterValue_InlinedBytes)
	if rhs := m.InlinedBytes; rhs != nil {
		tmpBytes := make([]byte, len(rhs))
		copy(tmpBytes, rhs)
		r.InlinedBytes = tmpBytes
	}
	return r
}
...
func (this *BlobParameterValue_InlinedBytes) StableEqualVT(thatIface isBlobParameterValue_BlobOneof) bool {
	that, ok := thatIface.(*BlobParameterValue_InlinedBytes)
	if !ok {
		return false
	}
	if this == that {
	...
	if string(this.InlinedBytes) != string(that.InlinedBytes) {
		return false
	}
	return true
}
...
func (this *BlobParameterValue_InlinedBytes) EqualVT(thatIface isBlobParameterValue_BlobOneof) bool {
	that, ok := thatIface.(*BlobParameterValue_InlinedBytes)
	if !ok {
		return false
	}
