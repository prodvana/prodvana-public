...
func (m *ImageDetails) validate(all bool) error {
	...
	if m.GetRegistryInfo() == nil {
		err := ImageDetailsValidationError{
			field:  "RegistryInfo",
			reason: "value is required",
		}
		if !all {
	...
	if all {
		switch v := interface{}(m.GetRegistryInfo()).(type) {
		case interface{ ValidateAll() error }:
			if err := v.ValidateAll(); err != nil {
	...
	} else if v, ok := interface{}(m.GetRegistryInfo()).(interface{ Validate() error }); ok {
		if err := v.Validate(); err != nil {
