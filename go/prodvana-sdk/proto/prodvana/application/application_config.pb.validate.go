...
func (m *ApplicationConfig) validate(all bool) error {
	...
	for idx, item := range m.GetPipelineTemplates() {
		_, _ = idx, item
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, ApplicationConfigValidationError{
						field:  fmt.Sprintf("PipelineTemplates[%v]", idx),
						reason: "embedded message failed validation",
						cause:  err,
					})
				}
	...
	for idx, item := range m.GetServiceTemplates() {
		_, _ = idx, item

		if item == nil {
			err := ApplicationConfigValidationError{
				field:  fmt.Sprintf("ServiceTemplates[%v]", idx),
				reason: "value is required",
			}
			if !all {
				return err
