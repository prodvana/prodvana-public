...
func (m *KeyDeliveryDecisionEvent) CloneVT() *KeyDeliveryDecisionEvent {
	...
	if rhs := m.StatusExplanationEntities; rhs != nil {
		tmpContainer := make([]*model.KeyDeliveryDecision_EntitySnapshot, len(rhs))
		for k, v := range rhs {
			tmpContainer[k] = v.CloneVT()
		}
...
func (this *KeyDeliveryDecisionEvent) StableEqualVT(that *KeyDeliveryDecisionEvent) bool {
	...
	for i, vx := range this.StatusExplanationEntities {
		...
		if p, q := vx, vy; p != q {
			if p == nil {
				p = &model.KeyDeliveryDecision_EntitySnapshot{}
			}
			if q == nil {
				q = &model.KeyDeliveryDecision_EntitySnapshot{}
			}
			if !p.StableEqualVT(q) {
				return false
			}
