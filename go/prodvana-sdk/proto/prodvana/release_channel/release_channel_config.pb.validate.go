...
func (m *ReleaseChannelConfig) validate(all bool) error {
	...
	for idx, item := range m.GetServiceInstanceProtections() {
		_, _ = idx, item
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, ReleaseChannelConfigValidationError{
						field:  fmt.Sprintf("ServiceInstanceProtections[%v]", idx),
						reason: "embedded message failed validation",
						cause:  err,
					})
				}
...
func (m *Precondition_ReleaseChannelStable) validate(all bool) error {
	...
	case *Precondition_ReleaseChannelStable_ReleaseChannel:
		...
		oneofStableOneofPresent = true
		// no validation rules for ReleaseChannel
	case *Precondition_ReleaseChannelStable_Selector:
		if v == nil {
			err := Precondition_ReleaseChannelStableValidationError{
				field:  "StableOneof",
				reason: "oneof value cannot be a typed-nil",
